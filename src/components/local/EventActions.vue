...
<template>
	<Flex align="center" :class="$style.wrapper">
		...
		<Button v-else @click.prevent="handleWithdraw" :type="btnType" size="small" :disabled="isWithdrawDisabled" block>
			<template v-if="successfulWithdrawal">Successfully withdrawn {{ successfulWithdrawal?.amount.toFixed(2) }} {{"MUSD"}}</template>
			...
			<template v-else-if="!isWithdrawing && positionForWithdraw">
				<Icon name="coins" size="16" />Withdraw {{ numberWithSymbol(positionForWithdraw.value, ",") }} {{"MUSD"}}
			</template>
