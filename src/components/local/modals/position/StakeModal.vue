 ...
 * Services
 */
import { flameWager, currentNetwork, analytics, placeBet, approveMUSD, getContractAddresses } from "@sdk"
...
const buttonState = computed(() => {
	...
	if (isApproving.value) return { text: "Approving MUSD..", disabled: true }
...
const handleUserTransactionConfirmation = async () => {
	...
	try {
        ...
        await approveMUSD(addresses.wager, amountWei)
			...
			<Flex direction="column" gap="4">
				...
				<Flex direction="column" gap="16" :class="$style.stake_block">
					...
					<Flex align="center" gap="24">
						<Flex wide :class="$style.field">
							...
							/>
							<input :value="`${amount.value} MUSD`" disabled :class="$style.behind_input" />
						</Flex>
				...
				<Flex direction="column" gap="16" :class="$style.payout_block">
					...
					<Flex>
						...
						<Text size="26" weight="600" color="tertiary"> &nbsp;MUSD </Text>
					</Flex>
				</Flex>
			</Flex>
